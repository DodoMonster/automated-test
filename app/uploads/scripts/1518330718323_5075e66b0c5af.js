...
function startJob(params, allParams) {
    (async (params) => {
        ...
        try {
            ...
            imgList.push(fileSavePath + 'home_' + timeStamp + '.jpg');
            console.log("点击首页的登录按钮");
            txtLog += '<p>2.点击首页的登录按钮</p>';
            await page.tap('.user-photo');
            await page.screenshot({
                path: fileSavePath + 'click-login-btn_' + timeStamp + '.jpg'
            });
            imgList.push(fileSavePath + 'click-login-btn_' + timeStamp + '.jpg');

            await page.waitFor(3000);
            console.log("输入账号密码" + JSON.stringify(params));
            txtLog += '<p>3.输入账号密码' + JSON.stringify(params) + '</p>';

            await page.screenshot({
                path: fileSavePath + 'login-page_' + timeStamp + '.jpg'
            });
            imgList.push(fileSavePath + 'login-page_' + timeStamp + '.jpg');
            ...
            imgList.push(fileSavePath + 'first-click-eye_' + timeStamp + '.jpg');

            console.log("第二次点击眼睛");
            txtLog += '<p>5.第二次点击眼睛</p>';
            await page.tap('#app > div > div > div > form.eui-login-form.login-member-form.eui--margin-t-20 > div:nth-child(4) > div > div > span');
            await page.screenshot({
                path: fileSavePath + 'second-click-eye_' + timeStamp + '.jpg'
            });
            imgList.push(fileSavePath + 'second-click-eye_' + timeStamp + '.jpg');
            ...
            txtLog += '<p>7.测试完毕~~~预示测试流程顺畅</p>';
            console.log(txtLog);
            console.log(imgList);
        } catch (e) {
            console.log(e);
            console.log("测试出错了" + JSON.stringify(e));
            txtLog += '<p>测试出错了 ' + +JSON.stringify(e) + '</p>';
        }
        await browser.close();
    })(params);
}
